...
const getData = (uri, data) => (findMatch(uri) ? data : []);

const findMatch = (haystack, needle = SESSION_UUID) => {
  // console.log("findMatch receives: ", { haystack, needle });
...
nock("https://api.veriff.internal")
  ...
  // )
  // .reply(() => [500, "There is something wrong with the service"])

  .get(
    /\/media-context\/[0-9A-F]{8}-[0-9A-F]{4}-[4][0-9A-F]{3}-[89AB][0-9A-F]{3}-[0-9A-F]{12}$/i
  )
  .reply((uri) => [200, getData(uri, mediaContextMockData)]);
